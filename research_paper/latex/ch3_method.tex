...
As there is no compliance records exists in the dataset, 1\% of records of 3,219 tax return details were extracted from dataset to create labelled anomalies, following the benchmark-construction logic of Emmott et al.~\cite{emmott2013systematic}. Five mechanisms reflecting recognised risk types that the file can represent were applied:

\begin{itemize}
    \item \textbf{A1 Work-related expense inflation} (salary above \$20,000). Work-related expenses raised to 25--45\% of salary amount (peer 99th percentile 24\%).
    \item \textbf{A2 Business expense inflation} (business income above \$5,000, expenses below income). Expenses raised to 150--300\% of business income (peer 99th percentile 220\%).
    \item \textbf{A3 Rental deduction inflation} (gross rent above \$5,000). Rental deductions raised to 300--500\% of gross rent (peer 99th percentile 295\%).
    \item \textbf{A4 Taxable-income identity violation} (income above \$30,000). Taxable income set to 40--70\% of the identity-implied amount, nothing else changed.
    \item \textbf{A5 Investment deductions without investment income} (no interest or dividends reported). Interest deductions have been reported for 15--35\% of income even though there was no related income.
\end{itemize}
